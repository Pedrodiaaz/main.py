...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_res:
        ...
        if st.session_state.inventario:
            ...
            busq_res = st.text_input("🔍 Buscar caja por código (ID Tracking):", placeholder="Escribe el código para filtrar...")
            if busq_res:
            ...
            for est_key, est_label in estados_log.items():
                df_f = df_res[df_res['Estado'] == est_key]
                st.markdown(f'<div class="state-header">{est_label} ({len(df_f)})</div>', unsafe_allow_html=True)
                if not df_f.empty:
                    st.table(df_f[['ID_Barra', 'Cliente', 'Peso_Almacen', 'Pago', 'Monto_USD', 'Abonado']])
                else:
                    st.caption(f"Sin mercancía en este estado.")
        else:
