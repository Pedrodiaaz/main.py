...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_reg:
        with st.form("reg_form", clear_on_submit=True):
            ...
            f_val_ini = st.number_input("Medida Inicial (Kg o ft³)", min_value=0.0)
    ...
    with t_val:
        ...
        if pendientes:
            ...
            v_real = st.number_input(f"Medida Real ({medida})", min_value=0.0, value=float(paq['Peso_Mensajero']))
    ...
    with t_aud:
        ...
        if ver_p:
            ...
            busq = st.text_input("🔍 Buscar Guía:")
            df_aud = pd.DataFrame(st.session_state.inventario)
            if busq: df_aud = df_aud[df_aud['ID_Barra'].astype(str).str.contains(busq, case=False)]
            st.dataframe(df_aud, use_container_width=True)
            if st.session_state.inventario:
                guia_ed = st.selectbox("Editar/Eliminar:", [p["ID_Barra"] for p in st.session_state.inventario])
                ...
                if paq_ed:
                    ...
                    with c3: n_med = st.number_input("Medida Real", value=float(paq_ed['Peso_Almacen']))
...
# --- 5. PANEL CLIENTE ---
    ...
    for p in mis_p:
        ...
        st.markdown(f'<div class="p-card">{icon} #{p["ID_Barra"]} - {p["Estado"]} | Deuda: ${(p["Monto_USD"]-p["Abonado"]):.2f}</div>', unsafe_allow_html=True)
