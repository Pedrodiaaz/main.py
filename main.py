...
if st.session_state.usuario_identificado and st.session_state.usuario_identificado.get('rol') == "admin":
    ...
    with t_aud:
        ...
        if ver_p:
            ...
            busq_aud = st.text_input("🔍 Buscar por Guía (Auditoría):")
    ...
    with t_res:
        ...
        if st.session_state.inventario:
            ...
            # --- BUSCADOR POR CÓDIGO ---
            busq_res = st.text_input("🔍 Buscar caja por ID / Tracking (Resumen):", placeholder="Escriba el código...")
            if busq_res:
            ...
            # --- SECCIÓN POR ESTADOS ---
            mapping = {
                "RECIBIDO ALMACEN PRINCIPAL": "📦 EN ALMACÉN",
                "EN TRANSITO": "✈️ EN TRÁNSITO",
                "ENTREGADO": "✅ ENTREGADO"
            }

            for est_interno, est_visible in mapping.items():
                df_f = df_res[df_res['Estado'] == est_interno]
                st.markdown(f'<div class="state-header">{est_visible} ({len(df_f)})</div>', unsafe_allow_html=True)
                if not df_f.empty:
...
# --- 5. PANEL DEL CLIENTE ---
    ...
    if not mis_p:
        st.info("No hay paquetes asociados a tu cuenta.")
    else:
        ...
        for i, p in enumerate(mis_p):
            with (col_paq1 if i % 2 == 0 else col_paq2):
                ...
                st.markdown(f"""
                    <div class="p-card">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            ...
                            <span style="background:#3b82f6; padding:2px 10px; border-radius:10px; font-size:0.8em;">{p['Pago']}</span>
                        </div>
                ...
                """, unsafe_allow_html=True)
                st.progress(min(abonado/total, 1.0) if total > 0 else 0.0)
                st.markdown(f"""
                        <div style="display: flex; justify-content: space-between; font-size: 0.85em; margin-top: 8px;">
                            <span>Abonado: <b>${abonado:.2f}</b></span>
                            <span style="color:#f87171;">Resta: <b>${max(total-abonado, 0.0):.2f}</b></span>
                        </div>
                    </div>
                """, unsafe_allow_html=True)
        ...
        with t1:
            ...
            if st.button("Iniciar Sesión", use_container_width=True):
                if le == "admin" and lp == "admin123":
                    ...
                    st.rerun()
                u = next((u for u in st.session_state.usuarios if u
