...
st.markdown("""
    ...
    /* PORTADA ESTILO FIGMA (Solo visible en Landing) */
    .hero-container {
        height: 60vh;
        width: 100%;
...
# --- 3. FUNCIONES DE DASHBOARD (RECUPERADAS TOTALMENTE) ---
        ...
        if st.checkbox("🗑️ Ver Papelera"):
            ...
            if st.session_state.inventario:
                ...
                if st.button("💾 Guardar Cambios"):
                    ...
                    guardar_datos(st.session_state.inventario, ARCHIVO_DB); st.rerun()
                if st.button("🗑️ Eliminar Guía (Enviar a Papelera)"):
...
def render_client_dashboard():
    ...
    for p in mis_p:
        with st.container():
            ...
            st.markdown(f'<div class="p-card">📦 <b>#{p["ID_Barra"]}</b> - {p["Estado"]}</div>', unsafe_allow_html=True)
...
if st.session_state.usuario_identificado is None:
    if st.session_state.landing_vista:
        st.markdown(f"""
            <div class="hero-container">
                ...
                <p style="font-size:26px; font-weight:300; font-style:italic;">"La existencia es un milagro"</p>
            </div>
        """, unsafe_allow_html=True)
        _, col_btn, _ = st.columns([1, 1, 1])
        if col_btn.button("🚀 INGRESAR AL SISTEMA", use_container_width=True):
    ...
    cols = st.columns([8, 2])
    cols[0].markdown(f"**Socio:** {st.session_state.usuario_identificado['nombre']}")
    if cols[1].button("CERRAR SESIÓN 🔒"):
